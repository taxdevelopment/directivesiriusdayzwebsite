    ...
    <div className={cn("z-50 flex-1 overflow-y-scroll", className)}>
      <motion.header
        style={{
          ...
          backgroundColor: useMotionTemplate`rgb(2 6 24 / ${useTransform(scrollYBoundedProgressDelayed, [0, 1], [1, 0.1])})`,
...
function useBoundedScroll(threshold: number) {
  ...
  useEffect(() => {
    return scrollY.on("change", (current) => {
      ...
      scrollYBounded.set(clamp(scrollYBounded.get() + diff, 0, threshold));
    });
